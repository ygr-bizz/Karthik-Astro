    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (this.mFillViewport && View.MeasureSpec.getMode(heightMeasureSpec) != 0 && getChildCount() > 0) {
            ...
            if (childSize < parentSpace) {
                child.measure(getChildMeasureSpec(widthMeasureSpec, getPaddingLeft() + getPaddingRight() + lp.leftMargin + lp.rightMargin, lp.width), View.MeasureSpec.makeMeasureSpec(parentSpace, BasicMeasure.EXACTLY));
            }
        }
    }
        ...
        if (actionMasked != 0) {
            if (actionMasked == 1) {
                ...
                if (activePointerIndex == -1) {
                    ...
                    if (this.mIsBeingDragged || Math.abs(deltaY3) <= this.mTouchSlop) {
                        deltaY = deltaY3;
                    } else {
                        ...
                        this.mIsBeingDragged = true;
                        if (deltaY3 > 0) {
                            deltaY = deltaY3 - this.mTouchSlop;
                        } else {
                            deltaY = deltaY3 + this.mTouchSlop;
                        }
                    }
                    if (this.mIsBeingDragged != 0) {
                        if (dispatchNestedPreScroll(0, deltaY, this.mScrollConsumed, this.mScrollOffset, 0)) {
                            int deltaY4 = deltaY - this.mScrollConsumed[1];
                        ...
                        if (overScrollByCompat(0, deltaY2, 0, getScrollY(), 0, range, 0, 0, true) && !hasNestedScrollingParent(0)) {
                        ...
                        if (canOverscroll) {
                            int deltaY5 = deltaY2 - this.mScrollConsumed[1];
                            ensureGlows();
                            int pulledToY = oldY + deltaY5;
                            if (pulledToY < 0) {
                                EdgeEffectCompat.onPull(this.mEdgeGlowTop, ((float) deltaY5) / ((float) getHeight()), motionEvent.getX(activePointerIndex2) / ((float) getWidth()));
                                ...
                                if (pulledToY > range2) {
                                    EdgeEffectCompat.onPull(this.mEdgeGlowBottom, ((float) deltaY5) / ((float) getHeight()), 1.0f - (motionEvent.getX(activePointerIndex3) / ((float) getWidth())));
