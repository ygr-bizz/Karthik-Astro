...
public class StaggeredGridLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    private int fill(RecyclerView.Recycler recycler, LayoutState layoutState, RecyclerView.State state) {
        ...
        while (true) {
            if (layoutState.hasMore(state)) {
                ...
                if (layoutState2.mLayoutDirection == z) {
                    ...
                    if (lp.mFullSpan != 0) {
                        i3 = getMinStart(defaultNewViewLine);
                    } else {
                        i3 = currentSpan.getStartLine(defaultNewViewLine);
                    }
                    end = i3;
                ...
                if (lp.mFullSpan != 0 && layoutState2.mItemDirection == -1) {
                    if (assignSpan) {
                        this.mLaidOutInvalidFullSpan = z;
                    } else {
                        if (layoutState2.mLayoutDirection == z) {
                            hasInvalidGap = areAllEndsEqual() ^ z;
                        } else {
                            hasInvalidGap = areAllStartsEqual() ^ z;
                        }
                        if (hasInvalidGap) {
                            LazySpanLookup.FullSpanItem fullSpanItem3 = this.mLazySpanLookup.getFullSpanItem(position);
                            if (fullSpanItem3 != null) {
                                fullSpanItem3.mHasUnwantedGapAfter = z;
                            }
                            this.mLaidOutInvalidFullSpan = z;
                        }
                    }
                ...
                if (!isLayoutRTL() || this.mOrientation != z) {
                    if (lp.mFullSpan != 0) {
                        i2 = this.mSecondaryOrientation.getStartAfterPadding();
                    } else {
                        i2 = (currentSpan.mIndex * this.mSizePerSpan) + this.mSecondaryOrientation.getStartAfterPadding();
                    }
                    int otherStart2 = i2;
                    ...
                    if (lp.mFullSpan) {
                        ...
                        otherEnd2 = this.mSecondaryOrientation.getEndAfterPadding() - (((this.mSpanCount - (z ? 1 : 0)) - currentSpan.mIndex) * this.mSizePerSpan);
                    }
                    otherEnd = otherEnd2;
                    otherStart = otherEnd2 - this.mSecondaryOrientation.getDecoratedMeasurement(view);
                }
                if (this.mOrientation == z) {
        ...
        if (diff > 0) {
            return Math.min(layoutState2.mAvailable, diff);
        }
        return 0;
    }
